...
class CpiCommand(abc.ABC):
    """Represent CPI commands conveyed as `rp.Task` "metadata" payloads."""

    # Initially, CpiCommand subclasses have no unique instance state, but future
    # extension could bundle the implementation task state in CpiCommand instances
    # to minimize external bookkeeping structures in the ScalemsMaster instance.
    # For instance, the CpiCommand instance could hold a Future for the command's
    # completion.
    ...
    def _command_name(cls) -> scalems.cpi.CpiOperation:
        # Note that this could be replaced by a keyword argument in the `class` definition,
        # processed with an updated `__init_subclass__(cls, command_name, **kwargs)`.
...
class CpiStop(CpiCommand):
    ...
    from the client and begin shutting down.

    Design note:
        We need to clarify the post-condition of this command.

        Through the regular Raptor task handling, we probably can't return any later
        than once we have toggled the state to refuse additional submitted work.
        If we even wanted to wait until Raptor has unregistered the *raptor_id*,
        we may have lost the machinery for `Master._result_cb()`.
    """
...
class CpiExitScope(CpiCommand):
    """Provide implementation for ExitScope in RP Raptor.

    When a EXIT_SCOPE is received, the Master will stop handling new compute tasks
    from the client. Tasks generated by other Tasks already in the queue will
    continue to be allowed until/unless a firmer "stop" command is received or
    until all work has been completed.

    When the work queue has been emptied, the Raptor session will shut itself down.
    """
    ...
    def launch(cls, manager: ScaleMSRaptor, task: TaskDictionary):
        ...
        assert cpi_call["operation"] == "exit_scope"
        scope_id = cpi_call["operand"]["scope_id"]
        task["stderr"] = ""
        task["stdout"] = f"CPI EXIT_SCOPE({scope_id}) received"
        try:
    ...
    def result_hook(cls, manager: ScaleMSRaptor, task: TaskDictionary):
        ...
        scope_id = cpi_call["operand"]["scope_id"]
        manager.stop()
        logger.debug(f"CPI EXIT_SCOPE({scope_id}) performed.")
...
def raptor():
    ...
    # WARNING: There is not (yet) a standard or documented facility to set this env var.
    # Note: We could pass the host as the value.
    # Note: I think this toggle should be even more ephemeral than the master config file;
    # maybe we can set it in the rp.Task environment from a client-side command line flag.
    if (_host := os.environ.get("SCALEMS_EXECUTOR_PYCHARM_PYDEVD_HOST")) is not None:
        # _host is usually either "host.docker.internal" or "127.0.0.1"
        # noinspection PyUnresolvedReferences
        import pydevd_pycharm

        pydevd_pycharm.settrace(_host, port=12345, stdoutToServer=True, stderrToServer=True)
...
class ScaleMSRaptor(rp.raptor.Master):
    ...
    def result_cb(self, tasks: typing.Sequence[TaskDictionary]):
        ...
        1. Tasks submitted to and completed by the collaborating Worker(s).
    ...
    def enter_scope(self, scope_id: str, worker_uids: typing.Sequence[str]):
        ...
        self._cpi_scopes[scope_id] = list(worker_uids)
