...
def from_raptor_task_metadata(obj: typing.Sequence[str, None | dict]) -> CpiCall:
    ...
    if operation == "exit_scope":
        return exit_scope(operand)
    if operation == "hello":
        return hello()
    if operation == "start_scope":
        return start_scope(operand)
    if operation == "stop":
        return stop()
    raise scalems.exceptions.APIError(f"CPI metadata not recognized: {obj}")
