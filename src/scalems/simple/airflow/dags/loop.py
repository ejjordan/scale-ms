...
from scalems.simple.airflow.dags.tasks import run_grompp
...
def run_mdrun(tpr_path: str, output_info) -> dict:
    ...
    output_files={'-x':os.path.join(out_path, 'result.xtc'),
...
def extract_final_dhdl_info(result) -> dict[str, int]:
    ...
    state_local = list(extract_dataframe(dhdl, headers=headers)['Thermodynamic state'])[-1]  # local index of the last state  # noqa: E501
...
with DAG('looper', schedule=None, start_date=timezone.utcnow(), catchup=False, render_template_as_native_obj=True) as dag:
    grompp_result = run_grompp("sys.gro", "outputs/grompp")
    mdrun_outputs_info = [(f"outputs/sim_{i}", i) for i in range(2)]
    mdrun_result = run_mdrun.partial(tpr_path=grompp_result['-o']).expand(output_info=mdrun_outputs_info)
    dhdl = mdrun_result.map(get_dhdl)
    dhdl_result = extract_final_dhdl_info.expand(result=dhdl)
    state = dhdl_result.map(get_state)
    dhdl_store = store_dhdl_results.partial(output_dir="outputs/dhdl", iteration=0).expand(dhdl=state)
